...
public class MinioUtil {
    ...
    public List<String> bucketObjectNameLists(String bucketName) {
        ...
        if (flag) {
            Iterable<Result<Item>> objectIterable = minioClient.listObjects(ListObjectsArgs.builder().bucket(bucketName).build());
    ...
    public boolean uploadFileToBucket(String bucketName, String objectName, InputStream in) {
        if (!bucketExists(bucketName)) {
            ...
            minioClient.putObject(PutObjectArgs.builder().bucket(bucketName).object(objectName).stream(in, in.available(), -1).build());
